...
int rectangle_XY[8],Set_rectangle_XY[24];		/* 矩形框坐标(左上、右上、左下、右下)  细分出的12个点 */
...
{
		/* 对上边框 */
		Set_RectangleXY[2] = (rectangle_XY[0]+rectangle_XY[2])/2;				/* 起始中间点X */
		Set_RectangleXY[3] = (rectangle_XY[1]+rectangle_XY[3])/2;				/* 起始中间点Y */

		Set_RectangleXY[0] = (rectangle_XY[0]+Set_RectangleXY[2])/2;		/* 起左点中间点X */
		Set_RectangleXY[1] = (rectangle_XY[1]+Set_RectangleXY[3])/2;		/* 起左点中间点Y */

		Set_RectangleXY[4] = (rectangle_XY[2]+Set_RectangleXY[2])/2;		/* 起右点中间点X */
		Set_RectangleXY[5] = (rectangle_XY[3]+Set_RectangleXY[3])/2;		/* 起右点中间点Y */

	
		/* 对右边框 */
		Set_RectangleXY[8] = (rectangle_XY[2]+rectangle_XY[6])/2;				/* 起始中间点X */
		Set_RectangleXY[9] = (rectangle_XY[3]+rectangle_XY[7])/2;				/* 起始中间点Y */

		Set_RectangleXY[6] = (rectangle_XY[2]+Set_RectangleXY[8])/2;		/* 起左点中间点X */
		Set_RectangleXY[7] = (rectangle_XY[3]+Set_RectangleXY[9])/2;		/* 起左点中间点Y */

		Set_RectangleXY[10] = (rectangle_XY[6]+Set_RectangleXY[8])/2;		/* 起右点中间点X */
		Set_RectangleXY[11] = (rectangle_XY[7]+Set_RectangleXY[9])/2;		/* 起右点中间点Y */


		/* 对下边框 */
		Set_RectangleXY[14] = (rectangle_XY[6]+rectangle_XY[4])/2;				/* 起始中间点X */
		Set_RectangleXY[15] = (rectangle_XY[7]+rectangle_XY[5])/2;				/* 起始中间点Y */

		Set_RectangleXY[12] = (rectangle_XY[6]+Set_RectangleXY[14])/2;		/* 起左点中间点X */
		Set_RectangleXY[13] = (rectangle_XY[7]+Set_RectangleXY[15])/2;		/* 起左点中间点Y */

		Set_RectangleXY[16] = (rectangle_XY[4]+Set_RectangleXY[14])/2;		/* 起右点中间点X */
		Set_RectangleXY[17] = (rectangle_XY[5]+Set_RectangleXY[15])/2;		/* 起右点中间点Y */		


		/* 对左边框 */
		Set_RectangleXY[20] = (rectangle_XY[4]+rectangle_XY[0])/2;				/* 起始中间点X */
		Set_RectangleXY[21] = (rectangle_XY[5]+rectangle_XY[1])/2;				/* 起始中间点Y */

		Set_RectangleXY[18] = (rectangle_XY[4]+Set_RectangleXY[20])/2;		/* 起左点中间点X */
		Set_RectangleXY[19] = (rectangle_XY[5]+Set_RectangleXY[21])/2;		/* 起左点中间点Y */

		Set_RectangleXY[22] = (rectangle_XY[0]+Set_RectangleXY[20])/2;		/* 起右点中间点X */
		Set_RectangleXY[23] = (rectangle_XY[1]+Set_RectangleXY[21])/2;		/* 起右点中间点Y */	
...
uint8_t Tracking_CNT,Tracking_Start_Flag,Tracking_Enable;			/* 进入跟踪部分的计时 */
